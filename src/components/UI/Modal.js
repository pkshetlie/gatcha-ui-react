    ...
    const handleChangeEvolution = useCallback(async (evolutionId) => {
        ...
        try {
            ...
            if (response && response.evolution_displayed?.image) {
                ...
                const newPortraitUrl = `${API_BASE_URL.replace('/api', '')}${response.evolution_displayed.portrait}`;

                setCurrentImageUrl(newImageUrl);

                updateAssistant(assistantId, newPortraitUrl, newImageUrl); // Met à jour l'état global
            } else {
